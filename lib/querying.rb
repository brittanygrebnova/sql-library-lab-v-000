...
def select_series_title_with_most_human_characters
  "SELECT Series.title, COUNT(Characters.species) FROM series INNER JOIN characters ON Series.id = Characters.series_id GROUP BY Characters.species ORDER BY COUNT(Characters.species) WHERE Characters.species = 'human' DESC LIMIT 1"
end
