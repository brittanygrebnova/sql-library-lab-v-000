...
def select_series_title_with_most_human_characters
  "SELECT Series.title, COUNT(Characters.species) FROM series INNER JOIN characters ON Series.id = Characters.series_id WHERE COUNT(Characters.species) = (SELECT MAX((species)) FROM characters)"
end
